...
export class StorageManager {
  constructor() {
    this._settings = null;
    this._settingsCachedAt = 0;
    this._settingsTTL = 30000; // 30 seconds TTL
    this._settingsPromise = null;
  }

  /**
   * Load settings (cached with TTL)
   */
  async getSettings() {
    const now = Date.now();
    if (this._settings && (now - this._settingsCachedAt) <= this._settingsTTL) {
      return this._settings;
    }

    // Prevent concurrent DB reads: reuse the pending promise
    if (this._settingsPromise) return this._settingsPromise;

    this._settingsPromise = db.getAllSettings().then((settings) => {
      this._settings = settings;
      this._settingsCachedAt = Date.now();
      this._settingsPromise = null;
      return settings;
    }).catch((err) => {
      this._settingsPromise = null;
