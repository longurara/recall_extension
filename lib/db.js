...
export async function updateCollection(id, updates) {
  ...
  const updated = { ...existing, ...updates };
  return createCollection(updated);
}
...
export async function deleteCollection(id) {
  // First remove collectionId from all snapshots in this collection
  const all = await getAllSnapshots();
  const toUpdate = all.filter(s => s.collectionId === id);
  for (const s of toUpdate) {
    await updateSnapshot(s.id, { collectionId: null });
  }
...
export async function getSnapshotsByCollection(collectionId) {
  const all = await getAllSnapshots();
  return all.filter(s => s.collectionId === collectionId);
}
...
export async function getReadLaterSnapshots() {
  const all = await getAllSnapshots();
  return all.filter(s => s.isReadLater === true);
}
...
export async function getUnreadSnapshots() {
  const all = await getAllSnapshots();
  return all.filter(s => s.isReadLater === true && !s.isRead);
}
